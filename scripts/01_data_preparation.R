...
# Health variables
health_vars <- c("health", "hlthhmp", "hltprbn", "hltprpa", "hltprpf", "hltprhc", "hltprhb",
                 "hltprdi", "hltprbp", "hltpral", "hltprsc", "hltprsd", "hltprsh" )
...
# 4. Composit scores codification ===============================================

ess_processed <- ess_selected %>% 
  mutate(
    mca_count = rowSums(select(., all_of(mca_vars)) == 1, na.rm = TRUE),
    
    mca_any = as.integer(rowSums(across(all_of(mca_vars)) == 1,
                                    na.rm = TRUE) >= 1),
    
    score_cardio = rowSums(select(., hltprhc, hltprhb, hltprdi) == 1, na.rm = TRUE),
    score_musculo = rowSums(select(., hltprbn, hltprpa, hltprpf) == 1, na.rm = TRUE),
    score_atopi_inflam = rowSums(select(., hltprbp, hltpral, hltprsc) == 1, na.rm = TRUE),
    domicil = ifelse(domicil > 5, NA, domicil),
